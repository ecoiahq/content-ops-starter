...
function ImageGalleryStaticGrid({ images = [], hasTopMargin, justifyContent = 'flex-start', hasAnnotations }) {
    ...
    return (
        <div
            className={classNames('w-full', 'flex', 'flex-wrap', 'items-center', mapStyles({ justifyContent }), { 'mt-12': hasTopMargin })}
...
function ImageGalleryAnimatedGrid({ images = [], motion, hasTopMargin, hasAnnotations }) {
    ...
    const imageGridStyle = {
        '--sb-slide-speed': `${images.length * 8}s`
    ...
    return (
        <div className="w-full">
            <div className={classNames(
                'sb-image-strip-wrap',
                'relative',
                'before:absolute before:h-full before:w-1/6 before:left-0 before:top-0 before:content-[""] before:bg-gradient-to-l before:z-10',
                'after:absolute after:h-full after:w-1/6 after:right-0 after:top-0 after:content-[""] after:bg-gradient-to-r after:z-10',
                { 'mt-12': hasTopMargin }
            )}>
                <div className={classNames('sb-image-strip', 'overflow-hidden whitespace-nowrap relative')} style={imageGridStyle}>
                    <div className={classNames(
                        ...
                        {[...Array(2)].map((_, idx) => (
                            <div key={idx} className="sb-image-strip-content flex justify-around" {...(hasAnnotations && { 'data-sb-field-path': '.images' })}>
                                {images.map((image, index) => (
                                    <ImageBlock key={`${idx}-${index}`} {...image} {...(hasAnnotations && { 'data-sb-field-path': `.${index}` })} />
                                ))}
                            </div>
                        ))}
                    </div>
